...
% Simulation settings
epoch   = datetime(2026, 2, 12, 0, 0, 0, 'TimeZone', 'UTC');
...
scParams.Cd       = 2.0;        % Drag coefficient [-]
scParams.Cr       = 1.5;        % SRP coefficient [-]
...
attParams.I      = diag([10, 15, 8]);  % Inertia tensor [kg·m²]
attParams.dipole = [0; 0; 0.1];        % Magnetic dipole [A·m²]
attParams.Kp     = 0.5;                % Proportional gain [N·m]
attParams.Kd     = 2.0;                % Derivative gain [N·m·s]
attParams.mode   = 'LVLH';             % Nadir pointing mode

% Generate complete truth trajectory
truth = generateTruthTrajectory(t, epoch, orbitalElems, scParams, attParams);

% Extract truth variables for convenience
qTrue       = truth.qTrue;        % Attitude quaternion (ECI to Body), 4xN
omegaTrue   = truth.omegaTrue;    % Angular velocity (body frame) [rad/s], 3xN
rECI        = truth.rECI;         % Position in ECI [m], 3xN
vECI        = truth.vECI;         % Velocity in ECI [m/s], 3xN
B_ECI       = truth.B_ECI;        % Magnetic field in ECI [nT], 3xN

% Rotate magnetic field to body frame for MAG sensor
...
% Compute specific force for accelerometer (gravity + perturbations)
g0             = 9.80665;  % [m/s²]
forceTrue_body = repmat([0; 0; -g0], 1, N);  % Simplified: constant gravity

fprintf('=== Truth Generation Complete ===\n');

%% ========================================================================
% 3. SENSOR MEASUREMENTS GENERATION (Corrupted with Realistic Errors)
% =========================================================================
...
% 4. INITIALIZE MEKF
% =========================================================================
ekf = initializeMEKF(IMU, MAG, STR);
...
%% ========================================================================
% 5. MEKF PROPAGATION AND UPDATE LOOP
% =========================================================================
fprintf('\n=== Running MEKF ===\n');
...
for k = 2:N
    %% PREDICTION STEP (Gyro propagation at IMU rate)
    ekf = predictMEKF(ekf, imuMeas.gyro.omegaBody(:,k), IMU.dt);
    
    %% CORRECTION STEP (Sensor updates at their respective rates)
...
% =========================================================================
saveFlag = 1;
plotEKFResults(t, truth, qEst, biasEst, P_hist, imuMeas, saveFlag);
