...
%   using eigenvalue decomposition. It minimizes Wahba loss function:
%       L(A) = 1/2 * Σ wᵢ·||bᵢ - A·rᵢ||²
%   where:
%       A : Attitude matrix        (DCM ECI to body)
...
[qEst, DCMEst, questInfo] = solveQUESTAttitude(meas, nSTR);
...
fprintf('\n--- Theoretical Performance Limits (CRLB) ---\n');
    ...
    elseif questInfo.residualMean > 100
        dominantNoise = 'Motion blur (dynamics-limited)';
    elseif questInfo.residualStd / questInfo.residualMean > 0.8
...
saveFlag = 1;
plotQUESTResults(meas, nSTR, STR, DCMTrue, DCMEst, ...
                 qTrue, qEst, questInfo, saveFlag);
